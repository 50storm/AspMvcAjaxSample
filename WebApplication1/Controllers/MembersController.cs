...
namespace WebApplication1.Controllers
{
    public class MembersController : Controller
    {
        ...
        public ActionResult AjaxCreate([Bind(Include = "Id,LastName,FirstName,Email,Birth,Married,Memo")] Member member)
        {
            if (ModelState.IsValid)
            {
                //db.Entry(member).State = ModelState.Add();
                //保存
                db.Members.Add(member);
                db.SaveChanges();            
        ...
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public ActionResult AjaxEdit([Bind(Include = "Id,LastName,FirstName,Email,Birth,Married,Memo")] Member member)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    //db.Entry(member).Reload();
                    //var memberData = db.Members.Find(member.Id);
                    //db.Entry(memberData).State = System.Data.Entity.EntityState.Modified;
                    //db.SaveChanges();

                    // https://msdn.microsoft.com/en-us/data/jj592904
                    //var entry = db.Members.Single();
                    ...
                    db.Entry(member).Reload();
                    //var OriMember = db.Entry(member).OriginalValues;
